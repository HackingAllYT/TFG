...
from src.config.text import TEXT, RESOLU, RESOLU_LIST


configFileRoute = 'src/config/config.ini'

config = configparser.ConfigParser()
config.read(configFileRoute)
...
class configurationModal(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.title(TEXT[self.config['INITIAL']['IDIOMA']]["Configuración"])
        ...
        self.button_image_2 = PhotoImage(
            file=self.relative_to_assets("galicia.png"))
        ...
        self.button_image_3 = PhotoImage(
            file=self.relative_to_assets("spain.png"))
        ...
        self.button_image_4 = PhotoImage(
            file=self.relative_to_assets("uk.png"))
        ...
        self.screenResolution = StringVar(
            self, str(RESOLU_LIST.index(self.config['INITIAL']['RESOLU']) + 1))
    ...
    *******************************************************************************
    ...
    def setSelectedItems(self, value: str = None, valueType: str = 'color'):
        ...
        if value:
            ...
            img = PhotoImage(
                file=self.relative_to_assets(aux[value][1])
            )
            aux[value][0]["image"] = img
            aux[value][0].image = img
        else:
            img = PhotoImage(
                file=self.relative_to_assets(aux[self.color][1])
            )
            aux[self.color][0]["image"] = img
            aux[self.color][0].image = img

            img = PhotoImage(
                file=self.relative_to_assets(aux[self.idioma][1])
            )
    ...
    *******************************************************************************
    ...
    def aplicar(self, event=None):
        ...
        # save to a file
        with open(configFileRoute, 'w') as configfile:
